...
def gamma_aug(img, mask, factor, **__):
    t = A.RandomGamma(gamma_limit=(int(factor), int(factor)+1), p=1.)
    img = np.array(img)
    img = t(image=img)['image']
    img = Image.fromarray(img)
    return img, mask

def grid_distortion(img, mask, factor, **__):
    t = A.GridDistortion(num_steps=5, distort_limit=factor, p=1.0)
    img = np.array(img)
    img = t(image=img)['image']
    img = Image.fromarray(img)
    return img, mask

def _gamma_level_to_arg(level, _hparams):
    r = random.randint(90,120)
    level = (level / (_LEVEL_DENOM-1)) * r
    return level,

def _grid_distortion_level_to_arg(level, _hparams):
    level = (level / (_LEVEL_DENOM+1)) * 0.5
    return level,
...
def _solarize_add_level_to_arg(level, _hparams):
    # range [0, 110]
    return int((level / _LEVEL_DENOM) * 180),
...
LEVEL_TO_ARG = {
    ...
    'Gamma': _gamma_level_to_arg,
    'GridDistortion': _grid_distortion_level_to_arg,
}
...
class AugmentOp:
    ...
    def __call__(self, img, mask):
        ...
        self.level_args = self.level_fn(magnitude, self.hparams) if self.level_fn is not None else tuple()
        return self.aug_fn(img, mask, *self.level_args, **self.kwargs)
